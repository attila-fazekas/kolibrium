...
import dev.kolibrium.selenium.decorators.DecoratorManager.withDecorators
...
class SingleElementDescriptorTest {
    ...
    fun `should throw exception when element not found within timeout`() {
        ...
        every { mockSearchContext.findElement(By.id("missing")) } throws NoSuchElementException("not found")
    ...
    fun `should always ignore NoSuchElementException in wait config`() {
        ...
        every { mockSearchContext.findElement(By.id("test")) } answers {
            calls++
            if (calls == 1) throw NoSuchElementException("not yet") else mockElement
        }
        every { mockElement.isDisplayed } returns true

        every { mockSearchContext.findElement(By.id("test")) } answers {
            throw NoSuchElementException("not yet")
