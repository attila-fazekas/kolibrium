...
private fun validatePathParameters(
    ...
    pathVariables.duplicateNames.forEach { duplicate ->
        errors +=
            Diagnostic(
                "Duplicate path variable '{$duplicate}' in ${info.path} — Kotlin can't have two constructor parameters with the same name",
                requestClass,
            )
    }
    ...
    pathProperties.forEach { property ->
        ...
        if (resolvedType.nullability == Nullability.NULLABLE) {
            errors +=
                Diagnostic(
                    "@Path parameter '$propertyName' must not be nullable — path parameters are structurally required in the URL",
                    property,
                )
        }
